...
TEST_F(ParserTest, parseStructOfStruct)
{
  ...
  ASSERT_TRUE(typeid(Struct) == typeid(*term));
  ASSERT_EQ("point(1, X, z(1, 2, 3))", term->symbol());
}
...
TEST_F(ParserTest, parseStructThreeArgs)
{
  ...
  Term *term = parser.createTerm();
  ASSERT_TRUE(typeid(Struct) == typeid(*term));
  ASSERT_EQ("point(1, X, z)", term->symbol());
}
...
TEST_F(ParserTest, parseListEmpty)
{
  ...
  Parser parser(scanner);
  Term *term = parser.createTerm();
  ASSERT_TRUE(typeid(List) == typeid(*term));
  ASSERT_EQ("[]", term->symbol());
}
...
TEST_F(ParserTest, parseVar)
{
  ...
  Parser parser(scanner);
  Term *term = parser.createTerm();
  ASSERT_TRUE(typeid(Variable) == typeid(*term));
  ASSERT_EQ("_date", term->symbol());
}
...
TEST_F(ParserTest, parseStructOfStructAllTheWay)
{
  ...
  Term *term = parser.createTerm();
  ASSERT_TRUE(typeid(Struct) == typeid(*term));
  ASSERT_EQ("s(s(s(s(1))))", term->symbol());
}
...
TEST_F(ParserTest, ListAsStruct)
{
  ...
  Struct *myStruct = dynamic_cast<Struct *>(term);
  ASSERT_EQ(2, myStruct->arity());
  ASSERT_EQ(".(1, [])", myStruct->symbol());
  ASSERT_EQ("1", myStruct->args(0)->symbol());
  ASSERT_EQ("[]", myStruct->args(1)->symbol());
}
...
TEST_F(ParserTest, ListAsStruct2)
{
  ...
  Struct *myStruct = dynamic_cast<Struct *>(term);
  ASSERT_EQ(2, myStruct->arity());
  ASSERT_EQ(".(2, .(1, []))", myStruct->symbol());
  ASSERT_EQ("2", myStruct->args(0)->symbol());
  ASSERT_EQ(".(1, [])", myStruct->args(1)->symbol());
}
