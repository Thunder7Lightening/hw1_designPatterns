...
TEST(Struct, match2)
{
  ...
  Term *datas1[1] = {&tom};
  vector<Term *> v1(datas1, datas1 + sizeof(datas1) / sizeof(Term *));
  //vector<Term *> v1 = {&tom};
...
TEST(Struct, nested_struct1)
{
	...
	Term *datas2[1] = {&X};
	vector<Term *> v2(datas2, datas2 + sizeof(datas2) / sizeof(Term *));
	Struct s2(Atom("s2"), v2); // s2(X)

	Term *datas1[1] = {&s2};
	vector<Term *> v1(datas1, datas1 + sizeof(datas1) / sizeof(Term *));
	Struct s1(Atom("s1"), v1); // s1(s2(X))
...
TEST(Struct, nested_struct_and_multiVariable)
{
	...
	Term *datas2[1] = {&Y};
	vector<Term *> v2(datas2, datas2 + sizeof(datas2) / sizeof(Term *));
	Struct s2(Atom("s2"), v2); // s2(Y)

	Term *datas1[2] = {&s2, &X};
	vector<Term *> v1(datas1, datas1 + sizeof(datas1) / sizeof(Term *));
	Struct s1(Atom("s1"), v1); // s1(s2(Y), X)

	X.match(&Y);
	X.match(new Atom("kent_beck"));
